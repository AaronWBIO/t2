...
<script type="text/javascript">
	$(document).ready(function() {
		...
		function env(dat){
			var rj = jsonF('<?= base_url(); ?>/Administration/users/save/<?= $users_id ?>',dat);
			// console.log(rj);
			var r = $.parseJSON(rj);
			// console.log(r);
			...
			if(r.ok == 1){
				$('#popUp').modal('toggle');
				load('#usersList','<?= base_url(); ?>/Administration/users/list',{text: '<?= $user["username"];?>'});
			}else if(r.ok == 2){
				var html = `
		...
		var rj = jsonF('<?= base_url(); ?>/general/general/employee_id',{employee_id:'<?= $user['ID_EMPLEADO'] ;?>'});
		var r = $.parseJSON(rj);
		if(r.status == 1){
	...
	<form id="nEmp">
		<table class="table borderless" border="0">
			...
			<tr>
				...
				<td>
					<input type="text" value="<?= set_value('name',$user['name']) ?>"
			...
			<tr>
				...
				<td>
					<input type="text"  value="<?= set_value('username',$user['username']) ?>"
			...
			<tr>
				...
				<td>
					<input type="password" value="" name="password" id="password" 
						class="form-control oblig" placeholder="Cambiar Contraseña" >
				</td>
				<td></td>
			</tr>
			<tr>
				...
				<td>
					<input type="text"  value="<?= set_value('email',$user['email']) ?>"
			...
			<tr>
				...
				<td>
					<select name="status" name="status" id="status" class="form-control">
						<option value="100" <?= intval($user['status']) == 100 ? 'selected' : ''; ?> >Activo</option>
						<option value="0" <?= intval($user['status'] == 0) ? 'selected' : ''; ?> >Inactivo</option>
					</select>
				</td>
				<td></td>
			</tr>
